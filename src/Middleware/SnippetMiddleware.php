...
use Quiksnip\Web\Utils\Session as QuiksnipSession;
...
class SnippetMiddleware
{
	public static function fetchSnippet(Request $req, Response $res,): void
	{
		...
		if (!$last_session) {
			$session_key = QuiksnipSession::createSession($snippet_data["id"]);
		} else {
			$session_key = $last_session["session_key"];
		}
		$req->append("session_url", QuiksnipSession::generateSessionURL($slug, $session_key));
